...
每个 Agent 系统在显得聪明之前，都要先回答一个朴素问题：谁负责接住请求？你在前端敲下的一句话，不能直接交给模型。它要先被登记成一次有 ID、有生命周期、能被流式观察的 **run**，模型才有机会开口。这一章看 DeerFlow 如何接住输入、创建 run、装入运行时上下文，再把执行事件持续流回前端。
...
- **`run_agent()` 这个函数，什么都管。** 它把图调用、持久化、流式发布、回滚、tracing、清理全塞在一条路径上——功能很全，但也因此又重又挤。改它之前，先认清你动的是哪一段。
