...
如果只看旧的实现印象，很容易把“应用表缺少正式 migration/upgrade 路径”列成持久化债务。按当前代码看，这件事已经不能再这么写：`persistence.bootstrap`、Alembic baseline、`0002_runs_token_usage`、幂等 column helper 都已经进入启动期路径。

这不等于持久化没有问题了。schema bootstrap 解决的是“表结构怎么创建和升级”；下面的 split-brain 风险解决的是“多个 runtime store 是否消费同一份持久化配置”。它们是两类问题，不要混在一起。
