...
station: 3
title: Tool assembly · the tool list is not the source of truth
sha: 0fb18e36
sourceRefs:
...
MCP tools are the clearest proof that the list is not the final answer. With `tool_search.enabled` on, MCP tools **do not** dump full schemas onto the model at once. The prompt lists only names, and the model calls `tool_search` for the schema when it needs one. The reason is pragmatic:
