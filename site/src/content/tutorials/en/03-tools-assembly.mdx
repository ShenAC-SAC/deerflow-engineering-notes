...
station: 3
title: Tool assembly · the tool list is not the truth
sha: 0fb18e36
sourceRefs:
...
Hold onto this: **this stop is not a function registry; it is a capability snapshot for one run.** Which tools come from config, which are built in, which get added, removed, or *deferred* because of the model, the sandbox, a skill, MCP, or subagent policy - only after that policy pass do you get the final list handed to `create_agent(..., tools=...)`.
...
The main entry is `get_available_tools()` in <SourceRef file="backend/packages/harness/deerflow/tools/tools.py" sha="0fb18e36" />. But note the boundary: **what it produces is not the final tool list. It is a candidate set.** It answers a deliberately limited question:
...
Then, back in `_make_lead_agent()` (<SourceRef file="backend/packages/harness/deerflow/agents/lead_agent/agent.py" sha="0fb18e36" />), the candidate set passes through two more gates: the skill-level `allowed_tools` filter and the MCP deferred-tool assembly. Visibility is **two-stage**: gather candidates, then apply policy.
...
- **Control-flow tool** — `ask_clarification`'s function body is only a placeholder; the real behavior is in `ClarificationMiddleware`, which intercepts the call and uses `Command(..., goto=END)` to halt the run until the user's next message. **Model user input as an interrupt, not as a tool blocking on a human.**
...
MCP tools are the clearest proof that the list is not the whole truth. With `tool_search.enabled` on, MCP tools **do not** dump full schemas onto the model at once. The prompt lists only names, and the model calls `tool_search` for the schema when it needs one. The reason is pragmatic:
