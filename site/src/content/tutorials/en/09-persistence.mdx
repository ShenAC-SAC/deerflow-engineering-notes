...
## Debts Already Paid Down

If you only remember an older implementation, it is tempting to list "no formal migration/upgrade path for application tables" as a persistence debt. In the current code, that is no longer accurate: `persistence.bootstrap`, the Alembic baseline, `0002_runs_token_usage`, and the idempotent column helper are all on the startup path.

That does not mean persistence has no remaining design issues. Schema bootstrap answers "how are tables created and upgraded?" The split-brain risk below answers "do all runtime stores consume the same persistence config?" These are different problems.
