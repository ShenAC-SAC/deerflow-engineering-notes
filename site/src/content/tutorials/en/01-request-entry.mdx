...
**`run_agent()`** — the background worker that actually does the work (<SourceRef file="backend/packages/harness/deerflow/runtime/runs/worker.py" sha="0fb18e36" />). Note the boundary: **it is not the agent. It is the worker that holds the graph while it runs.**
